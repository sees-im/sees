...
export const submitContactMessage = createServerFn({ method: "POST" })
  .inputValidator((input: { name: string; email: string; topic: string; message: string; turnstileToken: string }) =>
...
export const putStorjNote = createServerFn({ method: "POST" })
  .inputValidator((input: { vaultId: string; note: unknown }) =>
    z
      .object({ vaultId: z.string().min(8).max(128), note: noteSchema })
      .parse(input),
  )
  .handler(async ({ data }) => {
    const vaultId = safeVaultId(data.vaultId);
...
export const getStorjMeta = createServerFn({ method: "POST" })
  .inputValidator((input: { lookupId: string; turnstileToken: string }) => getMetaSchema.parse(input))
  .handler(async ({ data }) => {
    const { getRequest } = await import("@tanstack/react-start/server");
    const req = getRequest();
    const verified = await verifyTurnstileToken(
...
export const checkStorjName = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      await client.send(
        new GetObjectCommand({ Bucket: bucket, Key: `names/${data.nameId}.json` }),
      );
      return { taken: true };
    } catch {
      return { taken: false };
...
export const claimStorjName = createServerFn({ method: "POST" })
  .inputValidator((input: { nameId: string; turnstileToken: string }) => claimNameSchema.parse(input))
  .handler(async ({ data }) => {
    const { getRequest } = await import("@tanstack/react-start/server");
    const req = getRequest();
    const verified = await verifyTurnstileToken(
...
export const getShareStatus = createServerFn({ method: "POST" })
  .inputValidator((input: { shareId: string }) =>
    z.object({ shareId: shareIdSchema }).parse(input),
  )
  .handler(async ({ data }) => {
    const client = getStorjClient();
    const bucket = getBucket();
