...
export type UnlockResult =
  | { ok: true }
  | { ok: false; reason: "invalid" | "two-factor-required" | "two-factor-invalid" };
...
interface VaultContextValue {
  ...
  unlock: (vaultName: string, passphrase: string, turnstileToken: string, twoFactorCode?: string) => Promise<UnlockResult>;
...
async function syncMetaToStorj(vaultName: string | null, passphrase: string | null, meta: VaultMeta) {
  if (!vaultName || !passphrase) return;
...
export function VaultProvider({ children }: { children: ReactNode }) {
  ...
  const setSessionTimeoutMinutes = useCallback((value: number | "never") => {
    const next = value === "never" || SESSION_TIMEOUT_OPTIONS.has(value)
      ? value
      : DEFAULT_SESSION_TIMEOUT_MINUTES;
  ...
  const createVault = useCallback(async (name: string, passphrase: string, turnstileToken: string) => {
    const strength = passphraseStrength(passphrase);
    if (!strength.ok) {
      throw new Error(
        "Passphrase too short. Use at least 3 characters.",
      );
    }
  ...
  const changePassphrase = useCallback(
    async (oldPassphrase: string, newPassphrase: string) => {
      ...
      if (!strength.ok) {
        throw new Error(
          "New passphrase too short. Use at least 3 characters.",
        );
      }
  ...
  const destroyVault = useCallback(async () => {
    ...
    try {
      ...
      if (fpStr) {
        ...
        try { await purgeStorjVault({ data: { vaultId } }); } catch { /* ignore */ }
        try { await deleteStorjAudit({ data: { vaultId } }); } catch { /* ignore */ }
      }
      if (name && passphrase) {
        try {
          const lid = await lookupId(name, passphrase);
