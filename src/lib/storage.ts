...
export const localVaultStorage: VaultStorage = {
  ...
  async deleteFile(id) {
    write(FILES_KEY, read<StoredFile>(FILES_KEY).filter((x) => x.id !== id));
  },
  async getFile(id) {
  ...
  async deleteNote(id) {
    write(NOTES_KEY, read<StoredNote>(NOTES_KEY).filter((x) => x.id !== id));
  },
};
...
export function createLocalVaultStorage(vaultId: string): VaultStorage {
  ...
  return {
    ...
    async deleteFile(id) {
      write(filesKey, read<StoredFile>(filesKey).filter((item) => item.id !== id));
    },
    async getFile(id) {
    ...
    async deleteNote(id) {
      write(notesKey, read<StoredNote>(notesKey).filter((item) => item.id !== id));
    },
  };
}
