...
function AccessPage() {
  ...
  const [mode, setMode] = useState<"unlock" | "create">(requestedMode === "create" ? "create" : "unlock");
      ...
      if (mode === "create") {
        ...
        if (!result.ok) {
          if (result.reason === "two-factor-required") {
            ...
            setError("Enter the 6-digit authenticator code for this vault.");
            return;
          }
          if (result.reason === "two-factor-invalid") {
            setTwoFactorRequired(true);
            setError("That authenticator code did not match.");
            return;
          }
    ...
    <main className="access-page">
      ...
      <div className="access-page__layout">
        <section className="access-page__intro">
          ...
          <p>
            The key is created here, used here, and stays here. SEES only receives what your
            browser has already encrypted.
          </p>
          <div className="access-assurances">
            <span><Check className="size-3.5" /> No email or account</span>
            <span><Check className="size-3.5" /> No recovery backdoor</span>
            <span><Check className="size-3.5" /> No key leaves this device</span>
          </div>
        </section>
      ...
      <div className="access-page__footer">
        <span><ShieldCheck className="size-3.5 text-brand" /> AES-256-GCM</span>
  ...
  return (
    <form onSubmit={handleSubmit} className="access-card access-card--standalone">
      <div className="access-card__head">
        ...
        <div className="access-status"><span className="status-pulse" /> E2E</div>
      </div>
      ...
      <div className="field-group">
        <div className="field-label-row">
          ...
          {mode === "create" && nameStatus !== "idle" && (
            <span className={`name-status name-status--${nameStatus}`}>{statusLabel[nameStatus]}</span>
          )}
        </div>
        <input
          id="vault-id"
      ...
      <div className="field-group">
        <div className="field-label-row">
          ...
          {mode === "create" && passphrase && <span className="entropy-label">{strength.label}</span>}
        </div>
        <PasswordInput
      ...
      {mode === "unlock" && twoFactorRequired && (
        <div className="field-group">
          <div className="field-label-row">
            ...
            <span className="entropy-label">2FA</span>
          </div>
          <input
            id="two-factor-code"
            type="text"
      ...
      >
        <span>
          {busy ? "Deriving private key…" : mode === "create" ? "Create my vault" : "Unlock my vault"}
        </span>
